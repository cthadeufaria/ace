...
fsm_t fsm_s1, fsm_s2, fsm_config0, fsm_config1, fsm_config2, fsm_config3, 
fsm_pc, fsm_end ,fsm_control, fsm_led1, fsm_led2, fsm_led3, fsm_led4, fsm_led5, fsm_led6, fsm_led7, fsm_led_7_config_2;
...
void update_tis(){
  ...
  fsm_led_7_config_2.tis = cur_time - fsm_led_7_config_2.tes;
}

void led_7_config_1(fsm_t & fsm){
  switch (fsm.state)
  {
  case 0:
    if(fsm.tis > T && fsm_config1.state == 1 && fsm_config0.state == 1){
      LED_7 = 1;
      fsm.new_state = 1;
    }
    break;
  case 1:
    if(fsm_config1.state != 1){
      LED_7 = 0;
      fsm.new_state = 0;
    }
    else if(fsm.tis >= T&& fsm_config1.state == 1){
      LED_7 = 0;
      fsm.new_state = 2;
    }
    break;
  case 2:
    if(fsm_config1.state != 1 || fsm_config1.state != 0){
      LED_7 = 0;
      fsm.new_state = 0;
    }
    else if(fsm.tis >= T && fsm_config1.state == 1){
...
void led_7_config_2(fsm_t & fsm){
  switch (fsm.state)
  {
  case 0:
    if(fsm_config2.state == 0 && fsm_config0.state == 2){
      fsm.new_state = 1;
    }
    break;
  case 1:
    if(fsm_config0.state != 2){
      fsm.new_state = 0;
    }
    else if(fsm_config2.state == 1 && fsm_config0.state == 2){
      fsm.new_state = 2;
    }
    break;
  case 2:
    ...
    else if(fsm_config2.state == 2 && fsm_config0.state == 2){
      fsm.new_state = 3;
    }
    break;
  case 3:
    ...
    else if(fsm_config2.state == 0 && fsm_config0.state == 2){
      fsm.new_state = 3;
    }
    break;
  }
...
void led_7_config_2_act_1(fsm_t & fsm){
  switch (fsm.state)
  {
  case 0:
    if(fsm_led_7_config_2.state == 1 && fsm.tis > T){
      fsm.new_state = 1;
      LED_7 = 1;
    }
    break;
  case 1:
    if(fsm_led_7_config_2.state != 1){
      fsm.new_state = 0;
      LED_7 = 0;
    }
    else if(fsm_led_7_config_2.state == 2 && fsm.tis > T){
      fsm.new_state = 2;
      LED_7 = 0;
    }
    break;
