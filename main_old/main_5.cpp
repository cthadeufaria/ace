...
void evolve_led2(fsm_t &fsm, fsm_t &control, fsm_t &c0){
  switch(fsm.state){
    case 0:
      if(control.state==1 && cs==0)
        fsm.new_state=1;
      else if((control.state==12 && pause)||(cs==1 && c0.state == 1)){
        fsm.new_state=7;
      }      
      break;
    ...
    //  && (pause || (fsm_config0.state==1 && cs==1))
      ...
      else if((!pause && cs==0) || (cs==1 && c0.state!=1))
        fsm.new_state=0;
      break;
...
void evolve_led3(fsm_t &fsm, fsm_t &control, fsm_t &c0){
  switch(fsm.state){
    case 0:
      if(control.state==1 && cs==0)
        fsm.new_state=1;
      else if((control.state==13 && pause)||(cs==1 && c0.state==2)){
        fsm.new_state=7;
      }      
      break;
    ...
    //  && (pause || (fsm_config0.state==1 && cs==1))
      ...
      else if((!pause && cs==0) || (cs==1 && c0.state!=2))
        fsm.new_state=0;
      break;
...
void act_config2(fsm_t &fsm){
  fsm_config2.state=fsm.state;
}

void act_config3(fsm_t &fsm){
 fsm_config3.state=fsm.state;
}
