...
void evolve_end(fsm_t &fsm, fsm_t &control){
  switch (fsm.state){
    case 0:
      if(control.state==4 && cs==0)//control state final
        fsm.new_state=1;
      break;
    case 1:
      if(end==0 && cs==0)
...
void evolve_led2(fsm_t &fsm, fsm_t &control){
  switch(fsm.state){
    case 0:
      ...
      else if((control.state==12 && pause)||(cs==1 && fsm_config0.state==1)){
        fsm.new_state=7;
        LED_5 = 1;
      }      
      break;
    ...
    //  && (pause || (m==1 && cs==1))
      ...
      else if((!pause && cs==0) || (cs==1 && fsm_config0.state!=1))
        fsm.new_state=0;
      break;
...
void evolve_led3(fsm_t &fsm, fsm_t &control){
  switch(fsm.state){
    case 0:
      if(control.state==1 && cs==0)
        fsm.new_state=1;
      else if((control.state==13 && pause)||(cs==1 && fsm_config0.state==2)){
        fsm.new_state=7;
      }      
      break;
    ...
    //  && (pause || (m==1 && cs==1))
      ...
      else if((!pause && cs==0) || (cs==1 && fsm_config0.state!=2))
        fsm.new_state=0;
      break;
...
void loop() 
{
    ...
    if (now - last_cycle > interval) {
      ...
      */
      ...
      evolve_led2(fsm_led2, fsm_control);
      evolve_led3(fsm_led3, fsm_control);
