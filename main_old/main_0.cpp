  ...
  switch(fsm.state){
    case 0:
      if(control.state==1 && cs==0)
        fsm.new_state=1;
      else if(control.state==11||(cs==1 && m==0))
        fsm.new_state=7;
      break;
...
void evolve_led2(fsm_t &fsm, fsm_t &control){
  switch(fsm.state){
    case 0:
      if(control.state==1 && cs==0)
        fsm.new_state=1;
      else if((control.state==12 && pause)||(cs==1 && m==1))
        fsm.new_state=7;
      break;
...
void act_leds(){
  ...
  */  
    //led7
    if(fsm_end.state == 1|| fsm_led7.state == 1 || fsm_led7.state == 4 || fsm_led7.state == 5 || fsm_led7.state == 8 || fsm_led7.state == 10|| fsm_led7.state == 13){
