...
//m=1
  ...
  case 4:
    if(fsm.tis>=T && c0.state==1 && cs==1)
      fsm.new_state=5;
    else if((c2.state!=c2.new_state) && c0.state==1 && cs==1)
      fsm.new_state=3;
    else if(cs==1 && c0.state!=1)
      fsm.new_state=0;
    break;
  ...
  case 5:
    if((c2.state!=c2.new_state) && c0.state==1 && cs==1)
      fsm.new_state=3;
...
void act_leds(){
    ...
    {
      if(fsm_led7.tis+lum<T){
        LED_7=map(fsm_led7.tis+lum, 0, T, 255, 0);
